...
from skrendu_utils import create_log, write_log, load_params
...
def read_args():
  ...
  return os.path.join(params_dir, f"{args.f}.json"), os.path.join(results_dir, f"{args.f}.csv"), args.n, args.b
...
if __name__ == "__main__":
  data_path, log_path, new_params, batch_size = read_args()
  create_log(log_path)
  ...
  if params_exist and new_params or params_exist is False:
    ...
    write_params(data_path, params)

  result_dict = {}
  data = load_params(data_path)
  n_queries = len(data)
  ...
  write_log(log_path, data, result_dict)